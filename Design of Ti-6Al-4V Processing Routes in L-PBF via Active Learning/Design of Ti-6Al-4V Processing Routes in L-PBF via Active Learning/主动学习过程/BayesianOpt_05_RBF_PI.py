...
def main():
    ...
    output_dir = f"ex_05_output_{timestamp}"
    ...
    acq = ProbabilityOfImprovement(model, best_f=best_f, maximize=True)
    with torch.no_grad():
    ...
    recommended.to_csv(f'{output_dir}/recommended_5.csv', index=False)
    recommended.to_csv('./recommended_5.csv', index=False)
