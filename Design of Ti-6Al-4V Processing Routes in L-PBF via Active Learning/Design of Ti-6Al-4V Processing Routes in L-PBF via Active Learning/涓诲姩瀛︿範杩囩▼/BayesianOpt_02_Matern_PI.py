...
def main():
    ...
    output_dir = f"ex_02_output_{timestamp}"
    ...
    acq = ProbabilityOfImprovement(model, best_f=y_train.max().item(), maximize=True)
    with torch.no_grad():
    ...
    recommended.to_csv(f'{output_dir}/recommended_2.csv', index=False)
    recommended.to_csv('./recommended_2.csv', index=False)
