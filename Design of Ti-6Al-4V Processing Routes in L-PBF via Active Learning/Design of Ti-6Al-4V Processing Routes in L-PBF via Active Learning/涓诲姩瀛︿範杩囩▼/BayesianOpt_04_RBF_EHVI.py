...
def main():
    ...
    output_dir = f"ex_04_output_{timestamp}"
    ...
    recommended.to_csv(f'{output_dir}/recommended_4.csv', index=False)
    recommended.to_csv('./recommended_4.csv', index=False)

    # 绘制样本分布图
    plot_pareto_front(df_labeled, recommended, output_dir)
    
    print(f"\n推荐10个样本，已保存到 recommended.csv")
