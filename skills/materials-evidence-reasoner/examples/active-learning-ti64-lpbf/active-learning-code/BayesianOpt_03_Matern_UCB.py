...
def main():
    print("="*70 + "\nMatern2.5 + UCB1.5 贝叶斯优化\n" + "="*70)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"ex_03_output_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    df_labeled = pd.read_csv('labeled.csv')
    df_unlabeled = pd.read_csv('init_unlabeled.csv')
    X_labeled, y_labeled = df_labeled[['x', 'y']].values, df_labeled['z'].values
    ...
    X_train = torch.tensor(normalize_inputs(X_labeled, INPUT_BOUNDS), dtype=torch.float64)
    y_train = torch.tensor(standardize_outputs(y_labeled)[0], dtype=torch.float64).unsqueeze(-1)
    X_pool = torch.tensor(normalize_inputs(X_unlabeled, INPUT_BOUNDS), dtype=torch.float64)
    print("训练GP模型...")
    model = SingleTaskGP(X_train, y_train, covar_module=ScaleKernel(MaternKernel(nu=2.5, ard_num_dims=2)))
    fit_gpytorch_mll(ExactMarginalLogLikelihood(model.likelihood, model))
    acq = UpperConfidenceBound(model, beta=1.5, maximize=True)
    with torch.no_grad():
    ...
    recommended.to_csv(f'{output_dir}/recommended_3.csv', index=False)
    recommended.to_csv('./recommended_3.csv', index=False)
