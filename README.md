# Materials Evidence Reasoner 4.6.12

面向材料研发的“文献基准—本地实验—偏差诊断—机理验证—经验更新”Agent Skill。

它不是单纯的论文总结器，也不是自动实验平台。它把论文、专利、标准、实验记录、表格、曲线、表征图像和计算结果整理为可追溯的数据资产，判断结果是否真正偏离匹配参照，给出能够被实验推翻的机理解释，沿 PSPP 链追溯异常，并根据关键未知量推荐最小验证实验集，并把经过条件与证据审查的机制关系沉淀为可版本化 Mechanism Knowledge Graph。

## 30 秒使用

你可以直接上传或粘贴以下任意内容，不需要先整理成固定格式：

1. 论文、补充信息、专利、标准或 TDS；
2. Excel/CSV/JSON 实验记录、仪器导出或原始曲线；
3. SEM、TEM、XRD、Raman、谱图等图片；
4. 口语描述，例如“这一批导热率明显低于前三批，配方没改”；
5. 你想做的决策，例如“判断是否复现失败”“找最可能原因”“规划下一项实验”。

示例请求：

```text
请用这 4 篇论文建立 NCM811 的条件化容量先验，整理我上传的 3 个批次实验，先检查倍率、归一化和电池形式是否可比，再判断偏差、排序机理，并生成下一步验证方案和可视化报告。
```

Skill 会先整理输入，并把缺失信息分成：

- 必须补充的 blocker；
- 会降低结论强度但不阻断分析的信息；
- 可选增强信息。

默认不会要求用户先选择 Research/Industrial 模式，也不会因为少量非关键字段而停止工作。
...
- `materials-report.md`：行动优先的人类报告；
- `materials-result.json`：可验证的完整证据图；
- 若干 CSV：证据、样品/运行、偏差和缺失信息；
- `materials-dashboard.html`：无需服务器、双击即可打开的离线仪表盘；
- `error-budget.json/md`：有重复层级数据时生成的描述性误差预算与限制说明；
- `mechanism-export/`：机理图谱 JSON、节点/边 CSV 与 Graphviz DOT；
- `mechanism-audit.json/md`：图谱可追溯性、边界、falsifier、迁移与更新治理审计。
...
4.6.6 的视觉层采用 Apple HIG 风格原则而非复制品牌界面：清晰层级、克制留白、动态浅/深色、半透明导航材质、细分隔线、可回查的状态标签和温和动效。概览先呈现阻塞项与下一步行动，导航显示各视图条目数；窄屏表格和图形保持可读并允许横向查看。所有状态同时用文字/形状表达，不只依赖颜色；支持 `prefers-reduced-motion`、高对比和降低透明度回退。
