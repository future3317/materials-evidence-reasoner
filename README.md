...
<p align="center">
  <a href="skills/materials-evidence-reasoner/SKILL.md">Agent Skill</a> ·
  <a href="skills/materials-evidence-reasoner/references/output-schema.json">Output Schema</a> ·
  <a href="skills/materials-evidence-reasoner/viewer/index.html">Offline Viewer</a> ·
...
仓库现在包含一次真实的 **GPT-5.6-luna 对照实验**：同一份 `skills/materials-evidence-reasoner/examples/error-budget-demo.csv`、同一个科研问题、同一个模型，只改变是否加载 Materials Evidence Reasoner。无 Skill 组输出普通研究备忘录；使用 Skill 组读取 `skills/materials-evidence-reasoner/SKILL.md`，运行输入盘点和误差预算工具，生成并通过 canonical JSON 校验。
...
查看完整离线评估：[`skills/materials-evidence-reasoner/examples/skill-effect-real/evaluation.html`](skills/materials-evidence-reasoner/examples/skill-effect-real/evaluation.html)；机器评分与校验状态见 [`skills/materials-evidence-reasoner/examples/skill-effect-real/evaluation.json`](skills/materials-evidence-reasoner/examples/skill-effect-real/evaluation.json)；两组原始输出见 [`skills/materials-evidence-reasoner/examples/skill-effect-real/`](skills/materials-evidence-reasoner/examples/skill-effect-real/)。重新运行评分：

```bash
python skills/materials-evidence-reasoner/scripts/evaluate_real_skill_effect.py
```

底层的确定性合同回归仍保留在 `skills/materials-evidence-reasoner/scripts/compare_skill_modes.py`，用于不依赖模型的包级冒烟测试。
...
完整数据与评分口径见 [`docs/three-model-skill-ab-evaluation.md`](docs/three-model-skill-ab-evaluation.md)；实验排查与下一轮主动学习建议见 [`docs/active-learning-troubleshooting-and-next-experiment.md`](docs/active-learning-troubleshooting-and-next-experiment.md)。图表由 [`skills/materials-evidence-reasoner/scripts/plot_skill_ab_results.py`](skills/materials-evidence-reasoner/scripts/plot_skill_ab_results.py) 基于上述数据重新生成。
...
请生成中文报告、source-extraction/source-dashboard.html、materials-result.json 和 materials-dashboard.html；按 skills/materials-evidence-reasoner/references/output-schema.json 输出并运行 skills/materials-evidence-reasoner/scripts/validate_output.py。最后告诉我先看哪个文件、哪些结论需要人工回查、下一步如何接入本地实验。
```
...
只有文献时，结果不会证明你的实验已经复现或失败；它提供的是条件化文献先验、证据台账、候选机理、信息缺口和验证入口。更完整的单篇文献请求见 `skills/materials-evidence-reasoner/examples/literature-only-request.md`，用户阅读和后续提问规则见 `skills/materials-evidence-reasoner/references/literature-user-workflow.md`。
...
```bash
# 1. 盘点和规范化用户输入
python skills/materials-evidence-reasoner/scripts/prepare_intake.py <file-or-directory> --output intake-output

# 2. 可选：对长表重复数据生成描述性误差预算
python skills/materials-evidence-reasoner/scripts/analyze_error_budget.py repeats.csv --value measurement_value --unit MPa -o error-budget.json --report error-budget.md

# 3. 文献/文档先经过环境感知提取（PDF 默认 Docling，XML/JATS 使用结构化解析）
python skills/materials-evidence-reasoner/scripts/extract_sources.py --check-environment
python skills/materials-evidence-reasoner/scripts/extract_sources.py paper.pdf supplement.xml --output source-extraction

# 3b. 如果输入里有主动学习/贝叶斯优化代码和候选结果，额外生成画像与 Apple 风格交互页
python skills/materials-evidence-reasoner/scripts/profile_active_learning.py <active-learning-folder> --output active-learning-profile

# 4. Agent 按 SKILL.md 生成 materials-result.json

# 5. 校验结构、ID、证据引用和不可比闸门
python skills/materials-evidence-reasoner/scripts/validate_output.py materials-result.json

# 6. 从同一 JSON 生成行动优先报告和离线仪表盘
python skills/materials-evidence-reasoner/scripts/render_report.py materials-result.json -o materials-report.md
python skills/materials-evidence-reasoner/scripts/render_dashboard.py materials-result.json -o materials-dashboard.html

# 7. 导出、索引与审计证据约束机理图谱
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py export materials-result.json -o mechanism-export
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py index materials-result.json -o mechanism-index.json
python skills/materials-evidence-reasoner/scripts/audit_mechanism_graph.py materials-result.json -o mechanism-audit.json --report mechanism-audit.md

# 8. 可选：在本机 Chromium 中做交互与暗色模式 smoke test
python skills/materials-evidence-reasoner/scripts/smoke_test_viewer.py materials-dashboard.html --report viewer-smoke.json --screenshots viewer-smoke
```

`prepare_intake.py` 是确定性输入助手；`extract_sources.py` 是文献文件的运行环境入口；`profile_active_learning.py` 只做主动学习文件角色、代码依赖和候选/推荐分布的静态画像，不执行用户代码，也不把模型分数当测量；`analyze_error_budget.py` 只在重复层级明确时提供描述性方差分解，不替代计量学不确定度评定；Agent 负责材料语义、论文图表、异常传播链和机理推理；`validate_output.py` 是发布闸门；renderer 只负责呈现，不得提升结论等级。完整交接见 `skills/materials-evidence-reasoner/references/agent-output-contract.md`。
...
环境处理和来源锚点的详细合同见 `skills/materials-evidence-reasoner/references/source-extraction-contract.md`。

### 外部词典与任务意图维护

领域术语、同义词、排除词、歧义词和任务意图不写死在 SKILL 正文中。维护时编辑 `skills/materials-evidence-reasoner/references/adapter-routing-lexicon.json` 或 `skills/materials-evidence-reasoner/references/task-intent-lexicon.json`，按 `concept` 保持语义定义，再运行：

```bash
python skills/materials-evidence-reasoner/scripts/build_adapter_assets.py
python skills/materials-evidence-reasoner/scripts/validate_knowledge_assets.py
python skills/materials-evidence-reasoner/scripts/evaluate_routing.py --self-test
```

术语只用于召回；LLM 仍需结合实体、语境、中心性、极性和证据独立性决定 `load`、`candidate` 或 `skip`。维护规则见 `skills/materials-evidence-reasoner/references/routing-maintenance-contract.md`；任务意图结构见 `skills/materials-evidence-reasoner/references/task-intent-lexicon.schema.json`。
...
```text
skills/materials-evidence-reasoner/viewer/index.html
```
...
```bash
python skills/materials-evidence-reasoner/scripts/render_source_dashboard.py source-extraction/source-extraction.json -o source-extraction/source-dashboard.html
# 可选：在本机 Chromium 中做离线交互 smoke test
python skills/materials-evidence-reasoner/scripts/smoke_test_source_dashboard.py source-extraction/source-dashboard.html --chromium <path-to-chromium> --report source-dashboard-smoke.json
```
...
```bash
python skills/materials-evidence-reasoner/scripts/prepare_intake.py <active-learning-folder> --output intake-output
python skills/materials-evidence-reasoner/scripts/profile_active_learning.py <active-learning-folder> --output active-learning-profile
```
...
包内 `examples/active-learning-ti64-lpbf/` 示例还提供了 `test_function.py`，但它只是与现有 CSV 一致的
Müller–Brown 合成目标函数，用来演示主动学习流程，不代表 Ti-6Al-4V 实测性能。
运行完整示例时使用：

```bash
python "skills/materials-evidence-reasoner/examples/active-learning-ti64-lpbf/active-learning-code/run_all.py" --iterations 10
```

脚本会在示例目录下创建 `runs/run_<timestamp>/` 工作副本，并把每轮的新标签写到
`iterations/iteration_XX/`；原始 `labeled.csv`、候选池和代码不会被覆盖。单独测试 QBC 采样时，可运行：

```bash
python skills/materials-evidence-reasoner/examples/active-learning-ti64-lpbf/active-learning-code/run_sample_and_update.py \
  --input-dir skills/materials-evidence-reasoner/examples/active-learning-ti64-lpbf/active-learning-code \
  --qbc-file skills/materials-evidence-reasoner/examples/active-learning-ti64-lpbf/results/qbc_recommended.csv \
  --output-dir tmp/active-learning-iteration
```

默认角色是：`labeled.csv`=已有标签记录、`init_unlabeled.csv`=候选空间、`recommended_*.csv`/`qbc_recommended.csv`=模型推荐、`.py`=方法来源、`sample_distribution_*.png`=选择诊断图。`z` 的来源、x/y 的物理含义和单位仍必须由研究者确认；`acquisition_value` 不是性能值，`qbc_variance` 不是实验不确定度。只有新增独立实验/可信模拟证据后，推荐点才能写入正式 `property_records`。详细合同见 `skills/materials-evidence-reasoner/references/active-learning-contract.md` 与 `skills/materials-evidence-reasoner/references/active-learning-field-lexicon.json`。

可直接复制的 Agent 请求见 `skills/materials-evidence-reasoner/examples/active-learning-request.md`；它与 `skills/materials-evidence-reasoner/references/agent-output-contract.md` 对齐，要求先生成中间画像，再生成并校验 canonical `materials-result.json`。
...
异常链是一次诊断，PSPP map 是经验快照，Mechanism Graph 才是跨运行复用层。它不自动吸收模型常识，不按相似文本静默合并，也不会因为引用次数多而升级置信。详见 `skills/materials-evidence-reasoner/references/mechanism-graph-contract.md`。

常用命令：

```bash
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py summary materials-result.json
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py query materials-result.json --text "grain boundary diffusion" --json
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py export materials-result.json -o mechanism-export
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py diff old.json new.json -o mechanism-diff.json
```

只有明确批准的更新才可写入新 artifact：

```bash
python skills/materials-evidence-reasoner/scripts/mechanism_graph.py apply materials-result.json --update-id MU-001 -o materials-result-updated.json
```
...
- `skills/materials-evidence-reasoner/examples/cross-domain-input.csv`：电池、金属、聚合物、陶瓷和器件记录混合输入；
- `skills/materials-evidence-reasoner/examples/cross-domain-intake/intake-packet.json`：无损盘点和字段映射；
- `skills/materials-evidence-reasoner/examples/cross-domain-intake/intake-checklist.md`：缺失信息分级提醒；
- `skills/materials-evidence-reasoner/examples/cross-domain-intake/normalized-tables/`：保留原列的规范化副本。
...
├── README.md                        # 项目说明与使用入口
├── LICENSE                           # MIT 许可证
├── DATA-NOTICE.md                    # 示例数据与数据使用说明
├── docs/                             # 项目文档、评估说明与图表
└── skills/materials-evidence-reasoner/
    ├── SKILL.md                      # Agent 主控制协议
    ├── agents/                       # 可选界面元数据
    ├── references/                   # Schema、合同、词典和协议
    ├── scripts/
    ...
    ├── examples/                     # 示例输入、结果、评估产物与主动学习原始包
    │   └── active-learning-ti64-lpbf/
    ├── templates/                    # 实验记录与 intake 模板
    ├── viewer/                       # 离线 JSON/CSV 查看器
    └── tests/                        # Skill 包测试
```

## 输出接口与旧结果

从 Skill 开始执行时，Agent 应直接按照 `skills/materials-evidence-reasoner/references/output-schema.json` 生成 canonical JSON，并在交付前运行：

```bash
python skills/materials-evidence-reasoner/scripts/validate_output.py materials-result.json
```

`skills/materials-evidence-reasoner/references/output-field-aliases.json` 只处理旧结果的明确字段别名，例如 `hypothesis_id → id`、`title → statement`、`experiment_id → id`。它不放宽 Schema，也不把自然语言支持证据猜成 evidence ID。旧文件可先运行 `python skills/materials-evidence-reasoner/scripts/normalize_output.py result.json -o normalized-view.json` 供 viewer 阅读，但必须迁移并通过 canonical 校验后才能作为正式机器输出。
...
实际是否可用以 `python skills/materials-evidence-reasoner/scripts/extract_sources.py --check-environment` 为准；不要求一次安装所有领域工具。
...
```bash
python skills/materials-evidence-reasoner/scripts/validate_package.py
```
...
```bash
python skills/materials-evidence-reasoner/scripts/run_adversarial_tests.py
```
...
公开使用时请保留 `LICENSE` 与 `DATA-NOTICE.md`，并按 `skills/materials-evidence-reasoner/references/agent-output-contract.md` 使用完整目录；只复制 `SKILL.md` 会丢失确定性规范化、校验和离线可视化能力。
